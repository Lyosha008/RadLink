...
public class ForgeChunkManager
{
    ...
    private static final TicketType<TicketOwner<BlockPos>> BLOCK = TicketType.m_9462_("forge:block", Comparator.comparing(info -> info));
    private static final TicketType<TicketOwner<BlockPos>> BLOCK_TICKING = TicketType.m_9462_("forge:block_ticking", Comparator.comparing(info -> info));
    private static final TicketType<TicketOwner<UUID>> ENTITY = TicketType.m_9462_("forge:entity", Comparator.comparing(info -> info));
    private static final TicketType<TicketOwner<UUID>> ENTITY_TICKING = TicketType.m_9462_("forge:entity_ticking", Comparator.comparing(info -> info));
    ...
    {
        ForcedChunksSavedData data = level.m_8895_().m_164858_(ForcedChunksSavedData.m_293948_(), "chunks");
    ...
    {
        return forceChunk(level, modId, owner.m_20148_(), chunkX, chunkZ, add, ticking);
    }
          ...
          TicketType<TicketOwner<T>> type, Function<ForcedChunksSavedData, TicketTracker<T>> ticketGetter)
    {
        ...
        ForcedChunksSavedData saveData = level.m_8895_().m_164861_(ForcedChunksSavedData.m_293948_(), "chunks");
    ...
    public static void writeForgeForcedChunks(CompoundTag nbt, TicketTracker<BlockPos> blockForcedChunks, TicketTracker<UUID> entityForcedChunks)
    {
        if (!blockForcedChunks.isEmpty() || !entityForcedChunks.isEmpty())
        {
            ...
            writeForcedChunkOwners(forcedEntries, blockForcedChunks, "Blocks", Tag.f_178203_, (pos, forcedBlocks) -> forcedBlocks.add(NbtUtils.m_129224_(pos)));
            writeForcedChunkOwners(forcedEntries, entityForcedChunks, "Entities", Tag.f_178204_, (uuid, forcedEntities) -> forcedEntities.add(NbtUtils.m_129226_(uuid)));
            ...
            for (Map.Entry<String, Long2ObjectMap<CompoundTag>> entry : forcedEntries.entrySet())
            {
                ...
                forcedEntry.m_128359_("Mod", entry.getKey());
                ListTag modForced = new ListTag();
                modForced.addAll(entry.getValue().values());
                forcedEntry.m_128365_("ModForced", modForced);
                forcedChunks.add(forcedEntry);
            }
            nbt.m_128365_("ForgeForced", forcedChunks);
        }
    }
          ...
          Map<TicketOwner<T>, LongSet> forcedChunks, String listKey, int listType, BiConsumer<T, ListTag> ownerWriter)
    {
        for (Map.Entry<TicketOwner<T>, LongSet> entry : forcedChunks.entrySet())
        {
            ...
            for (long chunk : entry.getValue())
            {
                ...
                modEntry.m_128365_(listKey, ownerList);
            }
        }
    }
    ...
    {
        ListTag forcedChunks = nbt.m_128437_("ForgeForced", Tag.f_178203_);
        for (int i = 0; i < forcedChunks.size(); i++)
        {
            ...
            if (ModList.get().isLoaded(modId))
            {
                ListTag modForced = forcedEntry.m_128437_("ModForced", Tag.f_178203_);
    ...
    {
        ListTag forcedBlocks = modEntry.m_128437_(key, Tag.f_178203_);
        for (int k = 0; k < forcedBlocks.size(); k++)
        {
            blockForcedChunks.computeIfAbsent(new TicketOwner<>(modId, NbtUtils.m_129239_(forcedBlocks.m_128728_(k))), owner -> new LongOpenHashSet()).add(chunkPos);
        }
    }
    ...
    {
        ListTag forcedEntities = modEntry.m_128437_(key, Tag.f_178204_);
        for (Tag uuid : forcedEntities)
        {
            entityForcedChunks.computeIfAbsent(new TicketOwner<>(modId, NbtUtils.m_129233_(uuid)), owner -> new LongOpenHashSet()).add(chunkPos);
        }
    }
