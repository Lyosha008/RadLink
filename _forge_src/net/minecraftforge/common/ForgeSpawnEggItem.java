...
public class ForgeSpawnEggItem extends SpawnEggItem {
    ...
    private static final DispenseItemBehavior DEFAULT_DISPENSE_BEHAVIOR = (source, stack) -> {
        Direction face = source.f_301783_().m_61143_(DispenserBlock.f_52659_);
        EntityType<?> type = ((SpawnEggItem)stack.m_41720_()).m_43228_(stack.m_41783_());

        try {
            type.m_20592_(source.f_301782_(), stack, null, source.f_301784_().m_121945_(face), MobSpawnType.DISPENSER, face != Direction.UP, false);
        } catch (Exception exception) {
            DispenseItemBehavior.f_181892_.error("Error while dispensing spawn egg from dispenser at {}", source.f_301784_(), exception);
            return ItemStack.f_41583_;
        }

        stack.m_41774_(1);
        source.f_301782_().m_220407_(GameEvent.f_157810_, source.f_301784_(), GameEvent.Context.m_223722_(source.f_301783_()));
        return stack;
    };
    ...
    private static class ColorRegisterHandler {
        ...
        public static void registerSpawnEggColors(RegisterColorHandlersEvent.Item event) {
            MOD_EGGS.forEach(egg ->
                event.register((stack, layer) -> egg.m_43211_(layer), egg)
            );
        }
    }
}