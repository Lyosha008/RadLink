...
public class JsonCodecProvider<T> implements DataProvider
{
    ...
    @Override
    public CompletableFuture<?> m_213708_(final CachedOutput cache)
    {
        final Path outputFolder = this.output.m_247566_(this.packType == PackType.CLIENT_RESOURCES
                ? PackOutput.Target.RESOURCE_PACK
                : PackOutput.Target.DATA_PACK);
        ImmutableList.Builder<CompletableFuture<?>> futuresBuilder = new ImmutableList.Builder<>();

        gather(LamdbaExceptionUtils.rethrowBiConsumer((id, value) -> {
            final Path path = outputFolder.resolve(id.m_135827_()).resolve(this.directory).resolve(id.m_135815_() + ".json");
            ...
            */
            futuresBuilder.add(DataProvider.m_253162_(cache, encoded, path));
        }));
