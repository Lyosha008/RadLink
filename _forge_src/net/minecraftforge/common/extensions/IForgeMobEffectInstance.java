...
public interface IForgeMobEffectInstance {
    ...
    default boolean isCurativeItem(ItemStack stack) {
       return this.getCurativeItems().stream().anyMatch(e -> ItemStack.m_41656_(e, stack));
    }
    ...
    default void writeCurativeItems(CompoundTag nbt) {
       ...
       getCurativeItems().forEach(s -> list.add(s.m_41739_(new CompoundTag())));
       nbt.m_128365_("CurativeItems", list);
    }
}
