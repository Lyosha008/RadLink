...
public interface IForgeFriendlyByteBuf {
    ...
    default <T> void writeRegistryId(@NotNull IForgeRegistry<T> registry, @NotNull T entry) {
        ...
        self().m_130085_(name); //TODO change to writing a varInt once registries use id's
        self().m_130130_(reg.getID(entry));
    }
    ...
    default <T> T readRegistryId() {
        ResourceLocation location = self().m_130281_(); //TODO change to reading a varInt once registries use id's
        ForgeRegistry<T> registry = RegistryManager.ACTIVE.getRegistry(location);
        return registry.getValue(self().m_130242_());
    }
