...
public class ForgeGameTestHooks {
    ...
    public static String getTestTemplate(Method method, GameTest meta, String testName) {
        ...
        if (meta.m_177046_().indexOf(':') != -1)
            return meta.m_177046_();

        String template = null;

        if (meta.m_177046_().isEmpty())
            template = testName;
        else
            template = getPrefixed(method, meta.m_177046_());
    ...
    public static void addTest(Collection<TestFunction> functions, Set<String> classes, Set<String> filters, TestFunction func) {
        boolean allowed = filters.isEmpty() || filters.stream().anyMatch(f -> f.equals(func.m_128081_()) || func.m_128081_().startsWith(f + '.'));
        if (!allowed)
            return;

        functions.add(func);

        var batch = func.m_128081_();
        classes.add(batch);
