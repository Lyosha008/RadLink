...
public abstract class QuadLighter
{
    ...
    public final void process(VertexConsumer consumer, PoseStack.Pose pose, BakedQuad quad, int overlay)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            var shade = level.getShade(normals[i][0] / 127f, normals[i][1] / 127f, normals[i][2] / 127f, quad.m_111307_());
        ...
        var color = quad.m_111304_() ? getColorFast(quad.m_111305_()) : WHITE;
        consumer.m_85995_(pose, quad, brightness, color[0], color[1], color[2], lightmap, overlay, true);
    }
    ...
    private float[] getColorFast(int tintIndex)
    {
        ...
        {
            var packedColor = colors.m_92577_(state, level, pos, tintIndex);
    ...
    protected static int getLightColor(BlockAndTintGetter level, BlockPos pos, BlockState state)
    {
        ...
        int blockLight = Math.max(level.m_45517_(LightLayer.BLOCK, pos), level.m_8055_(pos).getLightEmission(level, pos));
