...
public enum VanillaGuiOverlay {
    ...
    SCOREBOARD("scoreboard", (gui, guiGraphics, partialTick, screenWidth, screenHeight) -> {
        ...
        if (scoreplayerteam != null) {
            var slot = DisplaySlot.m_293761_(scoreplayerteam.m_7414_());
            if (slot != null)
                objective = scoreboard.m_83416_(slot);
        }

        if (objective == null)
            objective = scoreboard.m_83416_(DisplaySlot.SIDEBAR);

        if (objective != null)
            gui.m_280030_(guiGraphics, objective);
    }),
